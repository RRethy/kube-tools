.PHONY: all test lint lint-fix build build-kubectl-x build-kubernetes-mcp build-celery build-k2 fmt vet tidy install uninstall clean help
...
help:
	...
	@echo "  build-k2             - Build the k2 binary"
...
lint:
	...
	cd k2 && go run github.com/golangci/golangci-lint/cmd/golangci-lint run --timeout 10m
...
lint-fix:
	...
	cd k2 && go run github.com/golangci/golangci-lint/cmd/golangci-lint run --fix --timeout 10m

# Build all binaries
build: tidy fmt build-kubectl-x build-kubernetes-mcp build-celery build-k2
...
install: build
	...
	@install -m 755 bin/k2 $(INSTALL_DIR)/k2
	...
	@echo "  k2 installed to $(INSTALL_DIR)/k2"
